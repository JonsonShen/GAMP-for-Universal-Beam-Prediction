...
semilogy(snr_dB_list,error_counter_genie/n_counter,char(Marker_list(1+(rx_cb-1)*3)),snr_dB_list,error_counter/n_counter,char(Marker_list(2+(rx_cb-1)*3)),snr_dB_list,error_counter_subset/n_counter,char(Marker_list(3+(rx_cb-1)*3)));

hold on;

end
xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;
legend('[Rx1]Known CB','[Rx1]Full set unknown CB','[Rx1]Subset unknown CB','[Rx2]Known CB','[Rx2]Full set unknown CB','[Rx2]Subset unknown CB','[Rx3]Known CB','[Rx3]Full set unknown CB','[Rx3]Subset unknown CB');title('Prediction error rate comparison');
