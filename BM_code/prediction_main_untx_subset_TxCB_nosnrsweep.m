...
n_aop = 8;
n_zop = 4;
tx_params_n = generate_tx_para(M, N,n_aop, n_zop, range_aop_fine, range_zop,aop_tx_fine(tx_cb),zop_tx);
...
semilogy(snr_dB_list,error_counter_genie/n_counter,char(Marker_list(1+(tx_cb-1)*3)),snr_dB_list,error_counter/n_counter,char(Marker_list(2+(tx_cb-1)*3)),snr_dB_list,error_counter_subset/n_counter,char(Marker_list(3+(tx_cb-1)*3)));
hold on;

end
xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;
legend('[CB1]Known CB','[CB1]Full set unknown CB','[CB1]Subset unknown CB','[CB2]Known CB','[CB2]Full set unknown CB','[CB2]Subset unknown CB');title('Prediction error rate comparison');
