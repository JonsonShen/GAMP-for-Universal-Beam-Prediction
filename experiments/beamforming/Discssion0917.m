...
%function x = generate_y(CDL_x, tx_params, rx_params, n_samples,snr_dB)
    ...
    gtx = zeros(n_cluster, n_beams);
    grx = zeros(n_cluster, 1);
    gtrx = zeros(n_cluster, n_beams);
    ...
    for i_sample = 1:n_samples
        ...
        % Tx beamforming gain
        for i_tx = 1:n_beams
            gtx(:, i_tx) = BM_gain(tx_params(i_tx,:), ZoD, AoD);
            y_samples(i_sample,i_tx) = max(gtx(:, i_tx)' * H * grx+normrnd(0,sig_std/(10^(snr_dB/10))),0);
            %y_samples(i_sample,i_tx) = gtx' * H * grx+normrnd(0,sig_std/(10^(snr_dB/10)));
            gtrx(:, i_tx) = grx .* gtx(:, i_tx);
        end
    end    
    x = y_samples; 
%end