...
% Y_sample_rx = generate_y(CDL_x,tx_params_rx_asp,rx_params,n_samples,snr_dB_training);
% Y_sample_rx_genie = generate_y(CDL_x,tx_params,rx_params,n_samples,snr_dB_training);
...
[y_max_sample, I_max_sample]=maxk(y_mean_rx_genie, 2);
...
k2_0_idx_list = [3 6 1 8 4 5 2 7]; %index of reported observations in set B
figure;
Marker_list={'-o','-+','-*','-x','-diamond','-square'};
k_max = 2;
for k2_0_length = 2:6
%k2_0_length = 3;

%for k_max = 1:3 %number of max beams reported to BS
...
for i_test = 1:n_test
    ...
    if k_max > 1
        ...
        temp_k0k2 = y_cov_Rxg(rxg_max_idx,I_max_prd_rx,k1+1:end);
        cov_k0k2 = temp_k0k2(1,:,:); 
        cov_k0k2_perm = cov_k0k2(:,[k2_0_idx k2_1_idx]);
    end
...
semilogy(snr_dB_list,error_counter_subset/n_counter,char(Marker_list(k2_0_length)));
hold on;

end
semilogy(snr_dB_list,error_counter/n_counter,'-hexagram');

xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;legend('2 Measurement Reported','3 Measurement Reported','4 Measurement Reported','5 Measurement Reported','6 Measurement Reported','Full Set Reported');title('Prediction error rate comparison');
%xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;legend('1 prediction Reported','2 predictions Reported', '3 predictions Reported');title('Prediction error rate comparison');
