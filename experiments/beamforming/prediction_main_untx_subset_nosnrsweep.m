...
csv_file = {'CDL_A.csv'};
CDL_x = load_CDL(char(csv_file),n_cluster);
%Rx side training
...
%semilogy(snr_dB_list,error_counter_genie/n_counter,char(Marker_list(1+(ch_i-1)*2)),snr_dB_list,error_counter_subset/n_counter,char(Marker_list(2+(ch_i-1)*2)));
%hold on;

%end
semilogy(snr_dB_list,error_counter_genie/n_test,'-o',snr_dB_list,error_counter/n_test,'-hex',snr_dB_list,error_counter_subset/n_counter,'-diamond',snr_dB_list,error_counter_rx/n_test,'-*',snr_dB_list,error_counter_sample/n_test,'-+');
xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;legend('Known codebook','Collaborative approach full set','Collaborative approach subset','Linear prediction','Legacy scheme');title('Prediction error rate comparison');
