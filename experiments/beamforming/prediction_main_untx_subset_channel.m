...
n_samples = 700; % number of samples used in training
n_test = 50000;
%n_AoA = 5;
%n_AoD = 5;
...
channel_file = {'CDL_A.csv','CDL_B.csv','CDL_C.csv'};
n_cluster = [20 22 22];

figure;
Marker_list={'-o','-+','-*','-x','-diamond','-square','-hexagram','-|','-^'};

Y_sample_rx = zeros(n_samples*length(snr_dB_list)*length(channel_file), length(tx_params(:,1)));
Y_sample_rx_genie = zeros(n_samples*length(snr_dB_list)*length(channel_file), length(tx_params(:,1)));

for ch_i = 1:length(channel_file)
csv_file = channel_file(ch_i);
CDL_x = load_CDL(char(csv_file),n_cluster(ch_i));
%Rx side training
...
for i_snr = 1:length(snr_dB_list)
    Y_sample_rx_snr = generate_y(CDL_x,tx_params_rx_asp,rx_params,n_samples,snr_dB_list(i_snr)); %standardized Tx codebook
    Y_sample_rx_genie_snr = generate_y(CDL_x,tx_params,rx_params,n_samples,snr_dB_list(i_snr)); %known Tx codebook
	Y_sample_rx(n_samples*(i_snr-1)*(ch_i-1)+1:n_samples*(i_snr-1)*(ch_i-1)+n_samples,:) = Y_sample_rx_snr;
	Y_sample_rx_genie(n_samples*(i_snr-1)*(ch_i-1)+1:n_samples*(i_snr-1)*(ch_i-1)+n_samples,:) = Y_sample_rx_genie_snr;
...
Y_sample_Rxg = zeros(n_rxg_hypo,n_samples*length(snr_dB_list)*length(channel_file),n_beams);
Y_sample_tx = zeros(n_rxg_hypo,n_samples*length(snr_dB_list)*length(channel_file),n_beams);
...
for i_rx_hypo = 1:n_rxg_hypo
	Y_sample_Rxg_temp = zeros(n_samples*length(snr_dB_list)*length(channel_file), length(tx_params(:,1)));
	Y_sample_tx_temp = zeros(n_samples*length(snr_dB_list)*length(channel_file), length(tx_params(:,1)));
    ...
    for i_snr = 1:length(snr_dB_list)
		...
		Y_sample_Rxg_temp(n_samples*(i_snr-1)*(ch_i-1)+1:n_samples*(i_snr-1)*(ch_i-1)+n_samples,:) = Y_sample_Rxg_snr;
		Y_sample_tx_temp(n_samples*(i_snr-1)*(ch_i-1)+1:n_samples*(i_snr-1)*(ch_i-1)+n_samples,:) = Y_sample_tx_snr;   
		% Y_sample_Rxg_snr = generate_y(CDL_x,tx_params_rx_asp,rx_params_hypo(i_rx_hypo,:),n_samples,snr_dB_list(i_snr));
		% Y_sample_tx_snr = generate_y(CDL_x,tx_params,rx_params_hypo(i_rx_hypo,:),n_samples,snr_dB_list(i_snr));
...
for i_test = 1:n_test
    ...
    error_counter(i_snr) = error_counter(i_snr)+nnz((I_max_prd_tx(1)-I_max_gt(1))*(I_max_prd_tx(2)-I_max_gt(1)));
    %error_acc = error_acc+norm(y_max_prd_tx-y_max_gt);

    error_counter_sample(i_snr) = error_counter_sample(i_snr)+nnz((I_max_sample(1)-I_max_gt(1))*(I_max_sample(2)-I_max_gt(1)));
    %error_acc_sample = error_acc_sample+norm(y_max_sample-y_max_gt);

    error_counter_genie(i_snr) = error_counter_genie(i_snr)+nnz((I_max_prd_rx_genie(1)-I_max_gt(1))*(I_max_prd_rx_genie(2)-I_max_gt(1)));
    error_counter_rx(i_snr) = error_counter_rx(i_snr)+nnz((I_max_prd_rx(1)-I_max_gt(1))*(I_max_prd_rx(2)-I_max_gt(1)));

    error_counter_subset(i_snr) = error_counter_subset(i_snr)+nnz((I_max_prd_subset(1)-I_max_gt(1))*(I_max_prd_subset(2)-I_max_gt(1)));
...
semilogy(snr_dB_list,error_counter_genie/n_counter,char(Marker_list(1+(ch_i-1)*3)),snr_dB_list,error_counter_subset/n_counter,char(Marker_list(2+(ch_i-1)*3)),snr_dB_list,error_counter_rx/n_counter,char(Marker_list(3+(ch_i-1)*3)));
hold on;

end

xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;
legend('[CDL-A]Known CB','[CDL-A]Subset unknown CB','[CDL-A]linear unknown CB','[CDL-B]Known CB','[CDL-B]Subset unknown CB','[CDL-B]linear unknown CB','[CDL-C]Known CB','[CDL-C]Subset unknown CB','[CDL-C]linear unknown CB');title('Prediction error rate comparison');
