...
k2_0_idx_list = [1 3 5 7 2 4 6 8]; %index of reported observations in set B
...
for i_test = 1:n_test
    ...
    % 9/9 Test
    r_k2_1 = quadprog(H_qua,f_qua,[],[],Aeq_qua,beq_qua,zeros(length(k2_1_idx),1)+0.0,10000000*ones(length(k2_1_idx),1));
    [x_opt] = active_set_method(H_qua, f_qua, Aeq_qua, beq_qua);
    [r_opt, fea ,test, one] = BoxCQP(H_qua, f_qua, Aeq_qua, beq_qua);
    if isempty(r_k2_1)
    ...
    if (x_opt == 1)
        x_opt = mean_k21_on_k20;
        count_x_fea = count_x_fea + 1;
    end
    if (fea == 1)
        r_opt = mean_k21_on_k20;
        count_r_fea = count_r_fea + 1;
    end
    if (norm(r_k2_1-x_opt)>1e-4)
        count_x_opt_error = count_x_opt_error + 1;
    end
    if (norm(r_k2_1-r_opt)>1e-4)
        count_r_opt_error = count_r_opt_error + 1;
    end
    if (test == 1)
        count_r_active = count_r_active + 1;
    end
    if (one == 1)
        count_r_one = count_r_one + 1;
    end
...
n_counter = n_test;
figure;
semilogy(snr_dB_list,error_counter_genie/n_counter,'-o',snr_dB_list,error_counter/n_counter,'-hex',snr_dB_list,error_counter_rx/n_counter,'-*',snr_dB_list,error_counter_sample/n_counter,'-+',snr_dB_list,error_counter_subset/n_counter,'-diamond');
xlabel('SNR(dB)');ylabel('Prediction error rate');grid on;grid minor;legend('Known codebook','Collaborative approach','Linear prediction','Legacy scheme','Subset');title('Prediction error rate comparison');
