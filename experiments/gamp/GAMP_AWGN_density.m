%% Simulation: Mismatch Rate vs. Sparsity Density (Fixed SNR + No Norm + AWGN)
% Goal: Compare GAMP strategies against LMMSE under varying Matrix Density.
%       **Fixed SNR = 20 dB**
%       **Observation Model: y_{k2} = G * y_{k1} + w (AWGN)**
%       **NO Column Normalization applied to G**
%       **Includes Variance Clipping & Strong Damping**
%       **G is FIXED for each density point across all trials**
% Output: A plot showing Mismatch Rate vs. Sparsity Density
...
% Sparsity Density Range for G
density_vec = 0.1:0.1:1.0;
num_points = length(density_vec);
...
% Results Storage
res_mean = zeros(1, num_points);
res_prob = zeros(1, num_points);
res_map  = zeros(1, num_points);
res_lmmse= zeros(1, num_points);

fprintf('Starting Simulation...\n');
fprintf('Fixed SNR: %d dB\n', SNR_dB);
fprintf('Signal Dimension (M): %d\n', M);
fprintf('Measurement Dimension (K): %d\n', K);
fprintf('Density Points: %s\n', mat2str(density_vec));
fprintf('Noise Type: Additive White Gaussian Noise (AWGN)\n');
fprintf('----------------------------------------\n');

total_timer = tic;

%% 2. Main Loop over Density
for d_idx = 1:num_points
    current_density = density_vec(d_idx);
    fprintf('Running Density = %.1f... ', current_density);
    
    % Reset counters
    errors_mean = 0; errors_prob = 0; errors_map = 0; errors_lmmse = 0;
    
    %% 2.1 FIXED transformation matrix G for this density point
    rng(d_idx,"twister");                        % fixed seed per density for repeatability
    G = sprandn(K, M, current_density);          % generate once per density (K x M)
    G = abs(G);
    ...
    % Precompute reused terms (do once per density)
    G2 = G.^2;
    G_full = full(G);
    ...
    for trial = 1:num_trials
        ...
        % AWGN Noise Calculation
        sigma2 = P_sig * 10^(-SNR_dB/10); % Noise Variance
        ...
        for t = 1:max_iter
            ...
            % 4. Input Nonlinear (Exp Signal - Unchanged)
    ...
    % Store
    res_mean(d_idx) = errors_mean / num_trials;
    res_prob(d_idx) = errors_prob / num_trials;
    res_map(d_idx)  = errors_map  / num_trials;
    res_lmmse(d_idx)= errors_lmmse/ num_trials;
...
%% 3. Plotting
figure('Name', 'Mismatch Rate vs Density', 'Color', 'w');
hold on;
grid on;
box on;

plot(density_vec, res_mean,  '-o', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'GAMP Mean');
plot(density_vec, res_prob,  '-s', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'GAMP Prob (Ranking)');
% plot(density_vec, res_map,   '-^', 'LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'GAMP MAP');
plot(density_vec, res_lmmse, '--x','LineWidth', 2, 'MarkerSize', 8, 'DisplayName', 'LMMSE (Baseline)');

xlabel('Sparsity Density of G (0.1 = Sparse, 1.0 = Dense)', 'FontSize', 12, 'FontWeight', 'bold');
ylabel('Mismatch Probability', 'FontSize', 12, 'FontWeight', 'bold');
title(sprintf('Mismatch Rate vs. Density (M =%d, K =%d, SNR =%d dB, AWGN)', M, K, SNR_dB), 'FontSize', 14);
legend('show', 'Location', 'best', 'FontSize', 10);
ylim([0, 1]);

%% --- Helper Functions ---
function [y_hat_new, vy_new] = estim_input_exponential(r_hat, vr, lambda)
    % Input estimator for Exponential Prior (UNCHANGED)
    % r_hat: effective observation
    % vr: effective noise variance
    % lambda: rate parameter of prior
    ...
    % Standard Mills ratio region
    if any(mask_normal)
        vals = alpha(mask_normal);
        h(mask_normal) = normpdf(vals) ./ normcdf(-vals);
    end
    
    % Approximation for large alpha to avoid NaN
    if any(mask_tail)
