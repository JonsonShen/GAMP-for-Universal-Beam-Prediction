%% Simulation: Mismatch Rate vs. Sparsity Density (Fixed SNR + No Norm)
...
%       **G is FIXED for each density point across all trials (random once OR custom)**
...
for d_idx = 1:num_points
    ...
    %% 2.1 FIXED transformation matrix G for this density point (choose ONE option)

    % -------------------- Option 1 (DEFAULT): Fixed random G (generate once per density) --------------------
...
title(sprintf('Mismatch Rate vs. Density (M =%d, K =%d, SNR =%d dB, Exp Noise)', M, K, SNR_dB), 'FontSize', 14);
